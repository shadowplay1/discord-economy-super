...
import { Client, ActivityType, Message, User, OAuth2Scopes } from 'discord.js'

import Economy from 'discord-economy-super/mongodb'
import { ShopItem, InventoryItem, HistoryItem } from 'discord-economy-super/mongodb/EconomyItems'
...
client.on('messageCreate', async msg => {
    ...
    const inventory = eco.cache.inventory.get<InventoryItem<CustomItemData>>({
        guildID: message.guild.id,
        memberID: message.author.id
    }) || []

    const history = eco.cache.history.get<HistoryItem<CustomItemData>>({
        guildID: message.guild.id,
        memberID: message.author.id
    }) || []
    ...
    console.log(await eco.database.fetch(`${message.guild.id}.${message.author.id}`))
    console.log(getUser(message.author.id).tag, user == undefined, 'GOVNOED')
    ...
    if (command == prefix + 'all') {
        message.channel.send(
            `\`\`\`js
${require('util').inspect(await eco.database.all())}\`\`\``)
    }
...
${require('util').inspect(result)}\`\`\``)
    ...
    if (command == prefix + 'transfer') {
        ...
        console.log(transferringResult);
    ...
    if (command == prefix + 'leaderboard') {
        const rawLeaderboard = await guild.leaderboards.balance()
    ...
    if (command == prefix + 'shop_find') {
        ...
        message.channel.send(
            ...
            `Name: ${item.name}` +
            `${item.custom.locked ? ` [🔒 | Locked since ${new Date(item.custom.lockedSince as number)
                .toLocaleString()}]` : ''}\n` +
    ...
    if (command == prefix + 'shop_edit') {
        ...
        await item.edit<any, any>(itemProperty, newValue)
        ...
        message.channel.send(
            ...
            `${hiddenShop
                .map((item, index) =>
                    `${index + 1} - ${item.custom.locked ? ' 🔒 | ' : ' '}${item.custom.emoji} ` +
                    `**${item.name}** (ID: **${item.id}**) - **${item.price}** coins ` +
                    `(Since **${new Date(item.custom.hiddenSince as number).toLocaleString()}**)`
                )
                .join('\n')}`
        )
    }
        ...
        message.channel.send(
            ...
            `${lockedShop
                .map((item, index) =>
                    `${index + 1} - ${item.custom.locked ? ' 🔒 | ' : ' '}${item.custom.emoji} ` +
                    `**${item.name}** (ID: **${item.id}**) - **${item.price}** coins ` +
                    `(Since **${new Date(item.custom.lockedSince as number).toLocaleString()}**)`
                )
                .join('\n')}`
        )
    }
    ...
    if (command == prefix + 'inventory') {
        ...
        const cleanInventory = [...new Set(userInventory.map(item => item.name))]
            .map(itemName => shop.find(shopItem => shopItem.name == itemName) as any as ShopItem<CustomItemData>)
            .map(item => {
