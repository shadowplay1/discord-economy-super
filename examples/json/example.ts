...
import { ShopItem } from 'discord-economy-super/EconomyItems'
...
client.on('messageCreate', async msg => {
    ...
    if (command == prefix + 'shop_add') {
        ...
        const newItem = guild.shop.addItem<CustomItemData>({
            name,
            price,
    ...
    if (command == prefix + 'shop_edit') {
        ...
        item.edit<any, any>(itemProperty, newValue)
    ...
    if (command == prefix + 'shop_buy') {
        ...
        const buyingResult = user.items.buy<CustomItemData>(item.id, quantity)
    ...
    if (command == prefix + 'shop_sell') {
        ...
        const sellingResult = user.items.sell<CustomItemData>(item.id, quantity)
